function percent = parfor_progress(N, progressPath)
...
%% path for evaluating progress
progressPath = fullfile(progressPath, 'parfor_progress.txt');

if N > 0
    f = fopen(progressPath, 'w');
    if f<0
...
elseif N == 0
    ...
    f = fopen(progressPath, 'a');
    fprintf(f, '1\n');
    fclose(f);
    
    f = fopen(progressPath, 'r');
    progress = fscanf(f, '%d');
    fclose(f);
